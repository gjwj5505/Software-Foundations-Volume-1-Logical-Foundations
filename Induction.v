...
Proof.
  ...
  - intros m. simpl. rewrite -> IHn'. f_equal.
Qed.
...
Proof.
  intros n. induction n as [| n' IHn'].
  - simpl. intros m. induction m as [| m' IHm'].
    + reflexivity.
    + simpl. rewrite <- IHm'. reflexivity.
  - simpl. intros m. induction m as [| m' IHm'].
    + rewrite IHn'. simpl. reflexivity.
    + simpl. f_equal. rewrite IHn'. rewrite <- IHm'. simpl. rewrite IHn'. reflexivity.
Qed.
...
Lemma double_plus : forall n, double n = n + n .
  Proof.
    intros n. induction n as [| n' IHn'].
    - simpl. reflexivity.
    (* - simpl. rewrite IHn'. symmetry. rewrite add_comm. simpl. reflexivity.  *)
    - simpl. rewrite IHn'. rewrite (add_comm n' (S n')). simpl. reflexivity.
  Qed.
(** [] *)
...
Proof.
  intros n. induction n as [| n' IHn'].
  - simpl. reflexivity.
  - assert (H1: even (S (S n')) = even n').
    { unfold even. reflexivity. }
    rewrite H1.
    assert (H2: even n' = negb (negb (even n'))).
    { rewrite negb_involutive. reflexivity. }
    rewrite H2. rewrite IHn'. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - intros n. simpl. rewrite mul_0_r. reflexivity.
...
Proof.
  ...
  - rewrite mul_comm. simpl. 
    rewrite mul_comm. simpl. 
    rewrite mul_comm. simpl. reflexivity.
  - rewrite mul_comm. simpl. 
    rewrite mul_comm.
    rewrite IHp'.
    rewrite !add_assoc.
    assert (H1: n + m + n * p' = n + n * p' + m).
      {
        rewrite <- add_assoc.
    ...
    rewrite H1.
    assert (H2: forall k l:nat, k * S l = k + k * l).
      {
        intros k l.
        rewrite mul_comm. simpl.
...
Theorem claude_mult_plus_distr_r : forall n m p : nat,
  (n + m) * p = (n * p) + (m * p).
Proof.
  intros n m p.
  induction n as [| n' IHn'].
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. reflexivity.
  - simpl. rewrite IHb'. simpl. f_equal.
...
(* 5을 표현하는 방법은 2진수로 101, 0101, 00101, 000101 등등 다양하게 존재. 이들을 구분하기 어려움.

0000101 -> 5 -> 101 로 변환되기 십상.
*)
...
Example normalize_0 : normalize (B0 Z) = Z.
Proof.
  simpl. reflexivity.
Qed.

Example normalize_00 : normalize (B0 (B0 Z)) = Z.
Proof.
  simpl. reflexivity.
Qed.


Example normalize_01 : normalize (B1 (B0 Z)) = B1 Z.
Proof.
  simpl. reflexivity.
Qed.

Example normalize_00101 : normalize (B1 (B0 (B1 (B0 (B0 Z))))) = B1 (B0 (B1 Z)).
Proof.
  simpl. reflexivity.
Qed.
...
Proof.
  intros b. induction b as [| b' IHb'| b' IHb'].
  - simpl. reflexivity.
  - simpl. rewrite <- add_comm. replace (bin_to_nat b' + 0) with (0 + bin_to_nat b'). 
    + simpl.
      rewrite double_nat_to_bin. simpl.
      rewrite IHb'.
      unfold double_bin.
