...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s with
  | nil => 0
  | n :: t => (if n =? v then 1 else 0) + (count v t)
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s with
  | nil => nil
  | n :: t => if n =? v then t else n :: (remove_one v t)
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s with
  | nil => nil
  | n :: t => (if n =? v then nil else [n]) ++ (remove_all v t)
  end.
...
Fixpoint included (s1 : bag) (s2 : bag) : bool :=
  match s1 with
  | nil => true
  | n1 :: t1 => if member n1 s2 then included t1 (remove_one n1 s2) else false
  end.
...
Qed.
*)
Theorem add_inc_count : forall (v: nat) (s: bag), count v (add v s) = 1 + (count v s).
Proof.
  intros v s.
  simpl. 
...
Proof.
  ...
  - simpl. intros l2. rewrite IHl'. rewrite app_assoc. reflexivity. 
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  match l1, l2 with
  | nil, nil => true
  | n1 :: t1, n2 :: t2 => (n1 =? n2) && (eqblist t1 t2)
  | _, _ => false
  end.
...
Proof.
  intros l. induction l as [| n l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite eqb_refl. simpl. rewrite IHl'. reflexivity.
Qed.
(** [] *)
...
(* FILL IN HERE
    ...
    [] *)
Theorem bag_count_sum : forall (v: nat) (s1: bag) (s2: bag),
  (count v s1) + (count v s2) = count v (sum s1 s2).
Proof.
  intros v s1. induction s1 as [|n1 s1' IHs1'].
  - intros s2. simpl. reflexivity.
  - intros s2. simpl. 
    destruct (n1 =? v).
    + rewrite <- add_assoc. rewrite IHs1'. reflexivity.
    + rewrite <- add_assoc. rewrite IHs1'. reflexivity.
Qed.
...
Proof.
  intros l. destruct l.
  - reflexivity.
  - intros default. simpl. reflexivity. 
Qed.
(** [] *)
